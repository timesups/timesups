<div align="center">

# Hi, I'm [timesups](https://github.com/timesups) 👋

[![GitHub followers](https://img.shields.io/github/followers/timesups?style=social)](https://github.com/timesups?tab=followers)
[![GitHub stars](https://img.shields.io/github/stars/timesups?style=social)](https://github.com/timesups)

专注于 **图形渲染**、**DCC 工具链** 与 **开发效率工具** 的开发者。

 [个人博客](https://timesups.github.io)

</div>

---

## 📊 GitHub 统计

<div align="center">

<img height="165" src="https://github-readme-stats.vercel.app/api?username=timesups&show_icons=true&theme=tokyonight&include_all_commits=true&count_private=true" alt="GitHub Stats" />
<img height="165" src="https://github-readme-stats.vercel.app/api/top-langs/?username=timesups&layout=compact&theme=tokyonight" alt="Top Languages" />

</div>

---
## 🛠️ 技术栈

| 领域 | 技术 |
|------|------|
| 图形与引擎 | C++ · OpenGL · RenderDoc |
| 脚本与工具 | Python |
| Web 前端 | TypeScript · HTML |
| DCC 管线 | Maya 插件 · AYON |
| 运维部署 | Docker · PowerShell |

---
## 📦 主要仓库

### 图形与渲染

- [GLEngineNew](https://github.com/timesups/GLEngineNew) — OpenGL 图形引擎 `C++`
- [RenderDocExporter](https://github.com/timesups/RenderDocExporter) — RenderDoc 导出工具 `Python`
- [SRR](https://github.com/timesups/SRR) — 图形渲染项目 `C++`

### Maya / DCC 工具

- [MayaPluginsCPP](https://github.com/timesups/MayaPluginsCPP) — Maya C++ 插件 `C++`
- [MayaPlugins](https://github.com/timesups/MayaPlugins) — Maya Python 插件 `Python`
- [MayaCharm](https://github.com/timesups/MayaCharm) — Maya 开发辅助工具 `Python`
- [XunFeiSDK](https://github.com/timesups/XunFeiSDK) — 讯飞 SDK 集成 `C++`

### AYON 管线

- [ayon-frontend](https://github.com/timesups/ayon-frontend) — AYON 服务端 Web 界面 `TypeScript`
- [ayon-backend](https://github.com/timesups/ayon-backend) — AYON 服务端 API `Python`
- [ayon-launcher](https://github.com/timesups/ayon-launcher) — AYON 桌面启动器
- [ayon-docker](https://github.com/timesups/ayon-docker) — AYON Docker 部署方案 `PowerShell`

### 工具与其他

- [FanMonitor](https://github.com/timesups/FanMonitor) — 风扇监控 `Python`
- [timesups.github.io](https://github.com/timesups/timesups.github.io) — 个人网站 `HTML`

---
<div align="center">

**感谢来访，欢迎 Star 与 Fork！**

</div>
